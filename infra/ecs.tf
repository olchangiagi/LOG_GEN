...
resource "aws_ecs_task_definition" "generator" {
  ...
  task_role_arn = aws_iam_role.esc_task_kinesis.arn
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        # 기본 초당 이벤트 발생량, 2.0: 초당 2개의 로그를 발생
        { name = "BASE_RPS", value = "2.0" },
        # 로그 생성 속도에 적용하는데이터 비율, 1.0: 정상, 2.0: 빠름, 0.5: 느림
        { name = "TIME_SCALE", value = "1.0" },
        # 의도적으로 생성하는 오염 데이터 비율. 0.03: 3% 오염데이터 생성 -> ETL에서 정제 처리 -> 97%만 사용
        { name = "CORRUPTION_RATE", value = "0.03" },
        # 데이터에 오염 데이터인지 아닌지 구분하는 라벨 표현, 표기 여부. false: 라벨 없음
        ...
        # 로그 생성의 실행 단위 구분용. manual, 20260820-001, 20260820-002...
