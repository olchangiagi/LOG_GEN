...
# 브론즈 kinesis 읽기 (입력, inputstream)
# 실버 kinesis 쓰기 (출력, outputstream)
# s3에 flink의 애플리케이션 zip 파일 읽기 -> 실행할 수 있음
# CloudWatch Logs에 로그 기록
data "aws_iam_policy_document" "flink" {
  # kinesis 읽기 권한 관련
  statement {
    sid    = "ReadBronzeKinesis"
    effect = "Allow"
    actions = [
  ...
  # silver level에 존재하는 kinesis로 데이터 전송(쓰기, 출력스트림)에 대한 권한
  statement {
  ...
  # s3에 존재하는 flink application 코드 (zip 형태로 구성)
  statement {
    sid    = "ReadFlinkCode"
    effect = "Allow"
    actions = [
...
# silver layer에서 사용하는 firehose role
