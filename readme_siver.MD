...
# parquet 저장 -> 스키마 필요 -> 열 검색, 기반 -> Glue 제공 (데이터베이스, 테이블, 크롤러, job 등을 다양하게 제공)
## Glue
- 데이터 관련 통합 서비스
- 데이터 구조 정의하며 이를 기반으로 쿼리 수행(Gold Layer에 적합한 데이터 형태로 추출(집계, 통계,... -> 분석))등 기반 제공
- 데이터베이스, 테이블, 스키마, 크롤러(데이터를 보고 자동으로 스키마 구성), ETL job 시각적 구성,...

## parquet
- 대규모 빅데이터를 효율적으로 저장하고 빠르게 좆회하기 위해 만든 열(column) 기반 오픈소스 파일 포맷
- 같은양의 데이터를 더 작은 공간 사용, 검색이 더 빠름, 호환성 높음, 

## 인프라 수정/추가
- Silver.tf의 firehose 저장 파트의 저장 부분 수정
  - silver layer를 통해 저장된 데이터 -> 현재 gzip파일, 대체: parquet 형태
  - compression_format = "GZIP" -> UNCOMPRESSED
  - 압축 담당 SNAPPY 담당
- Glue-silver.tf
  - s3 silver/.... 저장되는 parquet 데이터의 스키마 정의
  - json -> paquet 형식으로 변환할때 스키마 참조
  - Athena(raw 데이터, parquet등 압축 데이터를 대상으로 쿼리 가능)가 해당 파일을 대상으로 SQL 수행시 테이블/스키마 제공
  - s3 저장될 때, 파티션 형태로 저장(year/month/day/hour)된 형태에서 sql 수행 가능(파티션 활용)
- iam-flink.tf
  - 기존 role에 glue 권한 추가
- outputs.tf
  - glue 관련 이름, 리소스 등 출력
